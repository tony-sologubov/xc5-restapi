...
2.  On the setting page specify one of the following:
    ...
    *   **API key (only read)** - a key allowing an external application to only read data.
        ![](http://devs.x-cart.com/attachments/3768321/8718906.png?effects=drop-shadow)
...
4.  If product with ID = 1 exists in your database, you will get a result similar to the following:
    ...
    [JSON Product schema](http://devs.x-cart.com/attachments/code/product_schema.json){:target="_blank"} (this is product information in JSON format).
...
3.  `_path` param that identifies what entity you are pulling. This parameter is combined of an `[entity name](ref_RSR29iWL)` and an optional `entity ID` separated by the **/** character. Should contain `/0` (e.g. product/0) when used to create an entity with **POST** method.
...
* [Tony-RESTExtension-v5_2_0.tar](http://devs.x-cart.com/attachments/3768321/8718940.tar) (application/x-tar)
* [XCExample-RESTAPI-v5_3_0.tar](http://devs.x-cart.com/attachments/modules/XCExample-RESTAPI-v5_3_0.tar) (application/x-tar)
* [Example JSON Product schema](http://devs.x-cart.com/attachments/code/product_schema.json)
